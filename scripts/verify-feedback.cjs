...
vm.runInNewContext(compiled, {exports: route, require: name => name === '@/lib/app-version' ? {APP_VERSION:'1.03', IS_BETA:true} : require(name), process: {env}, Response, URL, Buffer, AbortSignal, fetch: async (url, options) => {calls.push({url, options}); return {ok:providerOk};}});
...
(async()=>{
 ...
 assert.match(mail.text,/Ver\.1\.03/); assert.match(mail.text,/送信日時:/); assert.match(mail.text,/現在ページ: \/\n/); assert.match(mail.text,/端末種別: iPhone/); assert.match(mail.text,/ブラウザ種別: Safari/);
 ...
 assert.match(component,/sending.current/); assert.doesNotMatch(component,/credentials:/);
